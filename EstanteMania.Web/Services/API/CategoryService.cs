...
namespace EstanteManiaWebAssembly.Services.API
{
    public class CategoryService : ICategoryService
    {
        ...
        public async Task<List<CategoryWithBooksDTO>?> GetAllCategoriesWithBooksAsync()
        {
            ...
            {
                var httpClient = _httpClientFactory.CreateClient("EstanteManiaApi");
                var response = await httpClient.GetAsync(apiEndpoint + "categories-with-books");

                if (response.IsSuccessStatusCode)
                {
                    return await response.Content.ReadFromJsonAsync<List<CategoryWithBooksDTO>?>();
                }
                else
                {
                    var message = await response.Content.ReadAsStringAsync();
                    _logger.LogError($"Error while trying to get categories. - {message}");
                    throw new Exception($"Status code: {response.StatusCode} - {message}");
                }
            }
            catch (UnauthorizedAccessException)
            ...
            {
                _logger.LogError($"Error while trying to get categories \n\n " + ex.Message);
                throw;
            }
        }
